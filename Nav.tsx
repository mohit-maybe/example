    ...
    <header className="sticky top-0 z-50 border-b-2 border-bone/30 bg-background/85 backdrop-blur">
      <div className="mx-auto flex max-w-[1400px] items-center justify-between px-6 py-4">
        ...
        <button
          ...
          className="brutal-border grid h-10 w-10 place-items-center text-bone md:hidden"
        >
      ...
      <AnimatePresence>
        {open && (
          <motion.nav
            ...
            className="overflow-hidden border-t border-bone/20 bg-background md:hidden"
          >
            <div className="flex flex-col px-6 py-2">
              {links.map((l) => {
                ...
                return (
                  <Link
                    key={l.to}
                    to={l.to}
                    className={`mono border-b border-bone/10 py-4 text-sm uppercase tracking-widest transition-colors ${
                      active ? "text-acid" : "text-bone/80 hover:text-bone"
                    }`}
                  >
                    {l.label}
                  </Link>
              ...
              <a
                href="mailto:mohitpreets67@gmail.com"
                className="mono py-4 text-xs uppercase tracking-widest text-blood"
              >
                Open · SEO + AI · 2026
              </a>
            </div>
